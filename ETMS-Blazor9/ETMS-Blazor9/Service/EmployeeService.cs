...
namespace ETMS.Data.Service
{
    public class EmployeeService : IEmployeeService 
    {
        ...
        //public EmployeeService(IDbContextFactory<ETMSContext> DbFactory, IEmployeeCourseService employeeCourseService)
        //{
        ...
        public async Task<List<Employee>> GetEmployees()
        {
            ...
            courseList = await _dbContext.Course.ToListAsync();
            courseList = courseList.OrderBy(c => c.Name).ToList(); 
            employeeCourseList = await _dbContext.EmployeeCourse.ToListAsync();
            employeeList = await _dbContext.Employee.ToListAsync();
        ...
        {
            return await _dbContext.Employee.FindAsync(int.Parse(id));
        }
        ...
        {
            int _id = int.Parse(id);
            var patient = await _dbContext.Employee.FindAsync(_id);
            if (patient == null)
            {
                return false;
            }

            _dbContext.Employee.Remove(patient);
            await _dbContext.SaveChangesAsync();
            return true;
        }
